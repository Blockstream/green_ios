...
function build() {
    ...
    export OPENSSL_PKG_CONFIG_PATH="$PWD/build-$1/openssl-1.0.2k/build/lib/pkgconfig"
...
if [ \( -d "$ANDROID_NDK" \) -a \( $# -eq 0 \) -o \( "$1" = "--ndk" \) ]; then
    ...
    function build() {
        ...
        export OPENSSL_PKG_CONFIG_PATH="$bld_root/openssl-1.0.2k/build/lib/pkgconfig"
