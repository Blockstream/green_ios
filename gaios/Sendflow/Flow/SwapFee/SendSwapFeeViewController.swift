...
class SendSwapFeeViewController: UIViewController {
    ...
    init?(coder: NSCoder, viewModel: SendSwapFeeViewModel) {
    ...
    func presentDialogCustomFeeViewController() {
        ...
        if let vc = storyboard.instantiateViewController(withIdentifier: "DialogCustomFeeViewController") as? DialogCustomFeeViewController {
            ...
            vc.delegate = self
            vc.feeRate = 0
            vc.minFeeRate = viewModel.minFeeRate()
            present(vc, animated: false, completion: nil)
        }
    }
}

extension SendSwapFeeViewController: DialogCustomFeeViewControllerDelegate {
    func didSave(fee: UInt64?) {
        if let fee {
            viewModel.select(priority: .Custom, feeRate: fee)
            viewModel.dismiss()
        }
    }
}

extension SendSwapFeeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: SwapFeeCell.identifier) as? SwapFeeCell {
            cell.selectionStyle = .none
            let priority = viewModel.priorities[indexPath.row]
            let model = SwapFeeCellModel(
                speedName: priority.text.localized,
                time: priority.time(isLiquid: viewModel.isLiquid).localized,
                feeRate: viewModel.feeRate(at: priority),
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let priority = viewModel.priorities[indexPath.row]
        let feeRate = viewModel.feeRate(at: priority)
        if let feeRate = feeRate {
            viewModel.select(priority: priority, feeRate: feeRate)
            viewModel.dismiss()
        }
    }
}
