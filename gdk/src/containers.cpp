...
namespace sdk {
    ...
    std::vector<subaccount_details> login_data::get_subaccounts() const
    {
        const auto subaccounts = get<std::vector<msgpack::object>>("subaccounts");
        std::vector<subaccount_details> p;
        p.reserve(subaccounts.size());
        std::copy(std::begin(subaccounts), std::end(subaccounts), std::back_inserter(p));
        return p;
    }

    subaccount_details login_data::get_subaccount(uint32_t subaccount) const
    {
        const auto subaccounts = get_subaccounts();
        auto p = std::find_if(std::begin(subaccounts), std::end(subaccounts),
            [subaccount](const subaccount_details& details) { return details.get<uint32_t>("pointer") == subaccount; });
        GA_SDK_RUNTIME_ASSERT(p != std::end(subaccounts));
        return *p;
    }
    ...
    void tx::construct_from(const msgpack::object& data)
    {
        associate(data);
        ...
        if (tx_in) {
            set("type", static_cast<uint32_t>(transaction_type::in));
        } else {
            received_on.clear();
            if (recipients.empty()) {
                set("type", static_cast<uint32_t>(transaction_type::redeposit));
            } else {
                set("type", static_cast<uint32_t>(transaction_type::out));
            }
        }
