...
namespace sdk {
    ...
    namespace {
        ...
        static nlohmann::json cleanup_utxos(nlohmann::json& utxos)
        {
            for (auto& utxo : utxos) {
                ...
                if (utxo_script_type == script_type::p2sh_p2wsh_csv_fortified_out) {
                    addr_type = address_type::csv;
                } else if (utxo_script_type == script_type::p2sh_p2wsh_fortified_out) {
                    addr_type = address_type::p2wsh;
                } else if (utxo_script_type == script_type::p2sh_fortified_out) {
                    addr_type = address_type::p2sh;
                } else if (utxo_script_type == script_type::pubkey_hash_out && external) {
        ...
        const std::string& private_key, const std::string& password, uint32_t unused)
    {
        ...
        wamp_call(
            ...
            "com.greenaddress.vault.get_utxos_for_script_hash", hex_from_bytes(script_hash_bytes));

        for (auto& utxo : utxos) {
            utxo["private_key"] = hex_from_bytes(private_key_bytes);
            utxo["compressed"] = compressed;
            utxo["public_key"] = hex_from_bytes(public_key_bytes);
            utxo["prevout_script"] = hex_from_bytes(script_bytes);
    ...
    nlohmann::json ga_session::get_receive_address(uint32_t subaccount, const std::string& addr_type_) const
    {
        ...
        GA_SDK_RUNTIME_ASSERT_MSG(is_known, "Unknown address type");
        ...
        GA_SDK_RUNTIME_ASSERT(address["address_type"] == addr_type);
